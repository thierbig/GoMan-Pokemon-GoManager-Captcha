        ...
        private static class ComputerFingerPrint
        {
            ...
            public static string Value()
            {
                if (string.IsNullOrEmpty(_fingerPrint))
                    _fingerPrint =
                        GetHash("CPU >> " + CpuId() + Constants.vbLf + "BIOS >> " + BiosId() + Constants.vbLf +
                                "BASE >> " + BaseId() + Constants.vbLf + "DISK >> " + DiskId() + Constants.vbLf +
                                "VIDEO >> " + VideoId());
                return _fingerPrint;
            }
            ...
            {
                if (string.IsNullOrEmpty(_fingerPrint))
                    _fingerPrint = "CPU >> " + CpuId() + Constants.vbLf + "BIOS >> " + BiosId() + Constants.vbLf +
                                   "BASE >> " + BaseId() + Constants.vbLf + "DISK >> " + DiskId() + Constants.vbLf +
                                   "VIDEO >> " + VideoId();
                return _fingerPrint;
            }
            ...
            private static string CpuId()
            {
                ...
                var retVal = identifierQuick("Win32_Processor", "UniqueId");
